...
function clientTimeZone() {
  try {
    ...
    let timezoneByCity = availableTimezones.find(
      (tz) => tz.caption.includes(clientGmtOffset) && tz.caption.toLowerCase().includes(tzCity)
    );
    ...
    return (
      availableTimezones.find((tz) => tz.name === 'UTC') || {
        name: 'UTC',
        caption: '(GMT+00:00) UTC',
      }
    );
