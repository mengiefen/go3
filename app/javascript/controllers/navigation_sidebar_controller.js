...
export default class extends Controller {
  ...
  connect() {
    console.log('NavigationSidebar controller connected');
  ...
  loadCollapsedState() {
    const savedState = localStorage.getItem('navigationSidebarCollapsed');
  ...
  saveCollapsedState(categoryIndex, isCollapsed) {
    ...
    localStorage.setItem('navigationSidebarCollapsed', JSON.stringify(collapsedState));
  }
  ...
  reset() {
    ...
    localStorage.removeItem('navigationSidebarCollapsed');
    this.categoryContentTargets.forEach((content, index) => {
      content.classList.remove('hidden');
      if (this.chevronTargets[index]) {
        this.chevronTargets[index].classList.add('rotate-90');
